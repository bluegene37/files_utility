...
void main() {
  ...
  group('checkForUpdate', () {
    ...
    test('throttles network calls within the check interval', () async {
      ...
      // still surfaces the available update.
      final result = await service(
    ...
    test('force surfaces a skipped version as available again', () async {
  ...
  group('downloadAsset', () {
    ...
    test('downloads bytes to a file and verifies the size', () async {
      ...
      final file = await UpdateService.downloadAsset(asset, dir: dir);
      expect(file.existsSync(), isTrue);
      expect(file.lengthSync(), bytes.length);
    });

    test('throws when the downloaded size does not match the asset', () async {
      final server = await HttpServer.bind(InternetAddress.loopbackIPv4, 0);
      server.listen((req) {
  ...
  group('windows install script', () {
    test('quotes paths and installs silently before relaunching', () {
      final script = UpdateService.buildWindowsInstallScript(
        installerPath: r'C:\Users\Jo Doe\AppData\Local\Temp\inst.exe',
        appExePath: r'C:\Program Files\Files Utility\files_utility.exe',
      );
      expect(
        script,
      ...
      final relaunchIdx = script.indexOf('files_utility.exe');
