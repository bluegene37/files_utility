...
class UpdateInfo {
  ...
  /// Parses [json] (the GitHub REST release object). Throws [FormatException]
  /// when the tag is not a version.
  factory UpdateInfo.fromReleaseJson(
    Map<String, dynamic> json,
    UpdatePlatform platform,
  ) {
    final info = tryFromReleaseJson(json, platform);
    if (info == null) {
      throw FormatException('Release tag is not a version: $json');
    }
    return info;
  }

  /// Like [UpdateInfo.fromReleaseJson] but returns null on unparseable tags,
  /// so a stray non-version release never breaks the update check.
