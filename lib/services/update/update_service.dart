...
/// [SharedPreferences] because it is app-global; the per-profile config DB
...
class UpdateService {
  UpdateService({
    ...
    this.checkInterval = const Duration(hours: 24),
       ...
       _fetchRelease = fetchRelease ?? _fetchReleaseHttp,
       _now = now ?? DateTime.now;

  static const String repoSlug = 'bluegene37/files_utility';
  ...
  static Future<Map<String, dynamic>> _fetchReleaseHttp(Uri url) async {
  ...
  /// the byte count against the size GitHub reported.
  static Future<File> downloadAsset(
    ...
    final targetDir =
        dir ?? await Directory.systemTemp.createTemp('files_utility_update_');
    final file = File(
